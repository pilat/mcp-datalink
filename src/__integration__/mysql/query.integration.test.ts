/**
 * Integration tests for query tool (MySQL)
 */
...
describe('query integration (MySQL)', () => {
  beforeEach(async () => {
  ...
  describe('basic queries', () => {
    it('executes basic SELECT', async () => {
      ...
      const result = await query(
        { database: 'mysqldb', sql: 'SELECT * FROM users ORDER BY name' },
        config
      );
    ...
    it('filters with WHERE clause', async () => {
      const config = createMySqlTestConfig();
      const result = await query(
        { database: 'mysqldb', sql: 'SELECT * FROM users WHERE is_active = TRUE' },
        config
      );
    ...
    it('respects explicit LIMIT', async () => {
      const config = createMySqlTestConfig();
      const result = await query(
        { database: 'mysqldb', sql: 'SELECT * FROM users LIMIT 1' },
        config
      );
    ...
    it('supports aggregate functions', async () => {
      const config = createMySqlTestConfig();
      const result = await query(
        { database: 'mysqldb', sql: 'SELECT COUNT(*) as count, SUM(balance) as total FROM users' },
        config
      );
  ...
  describe('data formatting', () => {
    ...
    it('formats boolean values', async () => {
      const config = createMySqlTestConfig();
      const result = await query(
        { database: 'mysqldb', sql: 'SELECT is_active FROM users ORDER BY name' },
        config
      );

      // MySQL returns TINYINT for BOOLEAN, so values will be 0 or 1
      const values = result.rows.map((r) => r[0]);
      expect(values).toContain('1'); // true
      expect(values).toContain('0'); // false
    });

    it('formats JSON values', async () => {
      const config = createMySqlTestConfig();
      const result = await query(
        {
          database: 'mysqldb',
  ...
  describe('truncation', () => {
    ...
    it('handles empty results', async () => {
      const config = createMySqlTestConfig();
      const result = await query(
        { database: 'mysqldb', sql: 'SELECT * FROM users WHERE 1=0' },
        config
      );
  ...
  describe('security - blocked queries', () => {
    it('blocks INSERT', async () => {
      ...
      await expect(
        query(
          { database: 'mysqldb', sql: "INSERT INTO users (id, email, name) VALUES ('test-id', 'test@test.com', 'Test')" },
          config
        )
    ...
    it('blocks UPDATE', async () => {
      ...
      await expect(
        query(
          { database: 'mysqldb', sql: "UPDATE users SET name = 'Hacked'" },
          config
        )
      ).rejects.toMatchObject({
        code: ErrorCode.QUERY_BLOCKED,
    ...
    it('blocks DELETE', async () => {
      ...
      await expect(
        query(
          { database: 'mysqldb', sql: 'DELETE FROM users' },
          config
        )
      ).rejects.toMatchObject({
        code: ErrorCode.QUERY_BLOCKED,
